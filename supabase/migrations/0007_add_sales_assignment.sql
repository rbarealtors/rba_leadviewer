alter table public.leads
  ...
  add column disposition_details text,
  add column next_follow_up timestamptz;

grant update (viewed_at, full_name, assigned_to, assigned_at, lead_status, disposition, disposition_details, next_follow_up) on public.leads to authenticated;
...
begin
  ...
  then
    raise exception 'Submitted lead fields are read-only; only mutable tracking fields may be updated';
  end if;
  return new;
end;
$$;
