...
export function LeadsClient({ initialLeads, kpiCounts, totalCount: initialTotalCount }: { initialLeads: Lead[], kpiCounts: any, totalCount: number }) {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          setToasts((prev) => [{ id: toastId, lead: newLead }, ...prev].slice(0, 3)); // Keep max 3 stacked
  ...
  function handleToastView(toastLead: Lead, toastId: string) {
    setSelectedLead(toastLead);
    if (!toastLead.viewed_at) {
      handleToggleViewed(toastLead);
    }
        ...
        >
          {toasts.map((toast) => {
            ...
            return (
              <div
                key={toast.id}
                className="pointer-events-auto bg-white dark:bg-zinc-900 border border-emerald-500/30 shadow-xl rounded-xl p-4 flex flex-col gap-2 transition-all transform duration-300"
              >
                {/* Header */}
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    className="text-subtle hover:text-ink p-1 rounded-md transition-colors cursor-pointer"
                    title="Dismiss"
                ...
                <div className="flex flex-col gap-1 text-xs">
                  ...
                  {cleanPhone ? (
                    <span className="text-subtle font-medium">{cleanPhone}</span>
                  ) : null}
                ...
                <div className="flex items-center gap-2 pt-1">
                  <button
                    ...
                    className="flex-1 bg-accent text-white hover:bg-accent-hover text-xs font-semibold py-1.5 px-3 rounded-lg transition-colors shadow-2xs text-center cursor-pointer"
                  >
                    View Lead
                  </button>
                  {waUrl && (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 bg-[#25D366] hover:bg-[#20bd5a] text-white text-xs font-semibold py-1.5 px-3 rounded-lg transition-colors shadow-2xs shrink-0 cursor-pointer"
                    >
